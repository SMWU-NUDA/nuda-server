...
import smu.nuda.domain.ingredient.service.IngredientAdminService;
...
public class AdminIngredientController {
    ...
    @Operation(
            summary = "성분 CSV 일괄 등록",
            description = "CSV 파일을 업로드하여 여러 성분 데이터를 한 번에 등록합니다."
    )
    @ApiResponses({
                                            ...
                                            value = """
                                                    {
                                                      ...
                                                      "data": "CSV 2번째 줄: external_product_id는 필수입니다."
                                                    }
                                                    """
                                    ),
                                    @ExampleObject(
                                            ...
                                            value = """
                                                    {
                                                      ...
                                                      "data": "CSV 4번째 줄: 매핑되는 상품이 존재하지 않습니다."
                                                    }
                                                    """
                                    ),
                                    @ExampleObject(
                                            name = "csvInvalidValue",
                                            summary = "유효하지 않은 컬럼 값",
                                            value = """
                                                    {
                                                      "success": false,
                                                      "code": "CSV_INVALID_VALUE",
                                                      "message": "컬럼 값이 올바르지 않습니다.",
                                                      "data": "CSV 6번째 줄: 존재하지 않는 layer_group 입니다."
                                                    }
                                                    """
                                    )
                            }
    ...
    public ApiResponse<CsvUploadResponse> uploadProducts(
        ...
        CsvUploadResponse res = ingredientAdminService.uploadIngredientsByCsv(csvFile, dryRun);
